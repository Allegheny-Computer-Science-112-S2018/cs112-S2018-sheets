...
experimentally study a complete implementation of the \program{ArrayList}
...
\program{ArrayList}'s constructor and \program{getFirst} method. Please
...
The key method that you must implement in the \program{ArrayList} is the
\program{toString} that produces a textual representation of the node-based data
structure. If the \program{ArrayList} contains a total of $n$ elements
...
\program{ArrayList}, denoted $e_n$, has a ``next'' reference that is
\program{null}. Once you are confident that your \program{toString} method
returns the correct textual representation of the \program{ArrayList} you
...
the questions about the \program{ArrayList}'s implementation, testing,
...
\item A properly documented and correct version of all the Java source files
  (e.g., \program{ArrayList} methods) that meets the set requirements and
