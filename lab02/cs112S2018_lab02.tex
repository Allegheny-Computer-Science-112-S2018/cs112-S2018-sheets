...
noticing that some of its methods are incomplete.
Next, you should also notice that the \mainprogram{} has a
test suite called \testprogram{} that is written in the JUnit testing framework;
please refer to Section 1.9 and the comment's in the source code of this test
...
\begin{figure}[t]
  ...
  \end{verbatim}
  \vspace*{-.5in}
  \caption{The output}~\label{fig:output}
  \vspace*{-.25in}
\end{figure}
...
\item Stored in a Markdown file called \reflection{}, a one-paragraph response, each consisting of at least three
  sentences, for each stated question.
